...
@Slf4j
@Service
public class SetWelcomeMediaOKHandler implements BaseBotMsgHandler {

    @Resource
    private ITgWelcomeConfigService welcomeConfigService;

    @Resource
    private SetWelcomeMediaHandler setWelcomeMediaHandler;
    ...
    public void process(BaseBot botAgent, Update update) {
        ...
        if(video==null && (photoList==null || photoList.size()==0) && document==null) {
            setWelcomeMediaHandler.process(botAgent, update);
            return;
        }
        ...
        try {
            ...
            org.telegram.telegrambots.meta.api.objects.File tgFile = botAgent.execute(getFile);
